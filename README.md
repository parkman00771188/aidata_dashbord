...
**검색 방식 (누락 최소화)**
- 매칭되는 **모든 행의 점수를 한 번의 스캔으로 계산**합니다. 예전에는 다운로드순 상위 900건만
  점수를 매겨서 관련도가 높아도 인기가 없으면 후보에 못 들어왔습니다(22,060건 중 4%만 검토).
- 흔한 낱말('등록', '위치')은 **IDF로 가중치를 낮춰** 정밀도를 지킵니다.
- **키워드마다 상위 20건을 반드시 확보**해 개념 하나가 통째로 빠지는 일을 막습니다.
- 질문에서 뽑은 토큰은 **조사를 떼어**('입지를'→'입지') 보조 검색어로 최대 4개만 씁니다.
- 같은 질문은 **키워드를 캐시**(`data/keyword_cache.json`)해 재검색 결과가 흔들리지 않습니다.
  (Gemini는 temperature 0에서도 키워드가 매번 달라져 결과가 바뀌는 원인이었습니다.)
- 검색에 2~7초가 걸리며, 결과 하단 회색 줄에 *매칭 N건 → 정밀검토 M건 → 후보 60건* 이 표시됩니다.
3. 결과 구성
